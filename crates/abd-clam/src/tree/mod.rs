    ...
    M: Fn(&I, &I) -> T,
{
        ...
        Ann: Fn(&Cluster<T, A>) -> A,
    {
        ...
        let mut frontier = vec![Cluster::new(0, 0, &mut items, &metric, strategy)];
        while let Some((mut cluster, splits)) = frontier.pop() {
            frontier.extend(splits.into_iter().map(|(child_center_index, c_items)| {
                // Create child cluster and get items for its children.
                let (mut child, child_splits) = Cluster::new(cluster.depth() + 1, child_center_index, c_items, &metric, strategy);
                // Set parent center index for child cluster.
                child.parent_center_index = cluster.center_index;
                (child, child_splits)
            }));
    ...
    M: Fn(&I, &I) -> T + Send + Sync,
{
        ...
        Ann: Fn(&Cluster<T, A>) -> A + Send + Sync,
    {
        ...
        // The `frontier` holds clusters that were just created but whose children have not yet been created.
        let mut frontier = vec![Cluster::par_new(0, 0, &mut items, &metric, strategy)];
        while let Some((mut cluster, splits)) = frontier.pop() {
            frontier.extend(
                splits
                    .into_par_iter()
                    .map(|(child_center_index, c_items)| {
                        // Create child cluster and get items for its children.
                        let (mut child, child_splits) = Cluster::par_new(cluster.depth() + 1, child_center_index, c_items, &metric, strategy);
                        // Set parent center index for child cluster.
                        child.parent_center_index = cluster.center_index;
                        (child, child_splits)
                    })
