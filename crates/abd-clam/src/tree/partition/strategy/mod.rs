...
impl<P> PartitionStrategy<P> {
        ...
        M: Fn(&I, &I) -> T,
    {
        ...
        let splits = if !matches!(self.max_split, MaxSplit::None) {
            self.max_split.split(metric, center_index, l_items, r_items, l_distances, r_distances)
        } else if !matches!(self.branching_factor, BranchingFactor::Unbounded) {
            self.branching_factor.split(metric, center_index, l_items, r_items, l_distances, r_distances)
        } else {
            self.span_reduction
                .split(metric, center_index, l_items, r_items, l_distances, r_distances, span)
        };
    ...
    P: Send + Sync,
{
        ...
        M: Fn(&I, &I) -> T + Send + Sync,
    {
        ...
        let splits = if !matches!(self.max_split, MaxSplit::None) {
            self.max_split.par_split(metric, center_index, l_items, r_items, l_distances, r_distances)
        } else if !matches!(self.branching_factor, BranchingFactor::Unbounded) {
            self.branching_factor
                .par_split(metric, center_index, l_items, r_items, l_distances, r_distances)
        } else {
            self.span_reduction
                .par_split(metric, center_index, l_items, r_items, l_distances, r_distances, span)
        };
