...
use distances_two::{blas, vectors};
...
fn blas_f32(c: &mut Criterion) {
    ...
    for d in 0..=3 {
        ...
        let id = BenchmarkId::new("L2Sq", dim);
        group.bench_with_input(id, &dim, |b, _| {
            b.iter_with_large_drop(|| {
                data.iter()
                    .map(|x| {
                        data.iter()
                            .map(|y| black_box(vectors::euclidean_sq(x, y)))
                            .collect::<Vec<_>>()
                    })
                    .collect::<Vec<_>>()
            });
        ...
        let id = BenchmarkId::new("BlasL2Sq", dim);
        group.bench_with_input(id, &dim, |b, _| {
            b.iter_with_large_drop(|| {
                data.iter()
                    .map(|x| {
                        data.iter()
                            .map(|y| black_box(blas::euclidean_sq(x, y)))
                            .collect::<Vec<_>>()
                    })
                    .collect::<Vec<_>>()
            });
        ...
        let id = BenchmarkId::new("L2", dim);
        group.bench_with_input(id, &dim, |b, _| {
            b.iter_with_large_drop(|| {
                data.iter()
                    .map(|x| {
                        data.iter()
                            .map(|y| black_box(vectors::euclidean(x, y)))
                            .collect::<Vec<_>>()
                    })
                    .collect::<Vec<_>>()
            });
        ...
        let id = BenchmarkId::new("BlasL2", dim);
        group.bench_with_input(id, &dim, |b, _| {
            b.iter_with_large_drop(|| {
                data.iter()
                    .map(|x| {
                        data.iter()
                            .map(|y| black_box(blas::euclidean(x, y)))
                            .collect::<Vec<_>>()
                    })
                    .collect::<Vec<_>>()
            });
...
criterion_group!(benches, blas_f32, blas_f64);
